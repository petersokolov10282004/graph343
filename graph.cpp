...
bool Graph::add(const string &label) {
  if(adjNodes.count(label) != 0) { return false;
}
  adjNodes[label] = {};
  return true; 
}
...
bool Graph::connect(const string &from, const string &to, int weight) {
  ...
  add(from);
  add(to);
  if(adjNodes.at(from).count(to) != 0) { return false;
}
  adjNodes[from][to] = weight;
  if(!directionalEdges) { adjNodes[to][from] = weight;
}
  return true;
}

bool Graph::disconnect(const string &from, const string &to) {
   if((adjNodes.count(from) == 0) || (adjNodes.count(to) == 0)) { return false;
}
   if(adjNodes.at(from).count(to) == 0) { return false;
}
   adjNodes[from].erase(to);
   if(!directionalEdges) { adjNodes[to].erase(from);
}
   return true;
}
...
void Graph::dfs(const string &startLabel, void visit(const string &label)) {
  ...
  while(!s.empty()){
    ...
    if(visited.count(current) == 0){
      ...
      for(auto it = adjNodes.at(current).rbegin(); it!= adjNodes.at(current).rend(); ++it) {
...
void Graph::bfs(const string &startLabel, void visit(const string &label)) {
  if(adjNodes.count(startLabel) == 0) { return;
}
  set<string> visited;
  queue<string> q;
  q.push(startLabel);
...
Graph::dijkstra(const string &startLabel) const {
  ...
  set<string> visited;
  if(adjNodes.count(startLabel) == 0) { return make_pair(weights, previous);
}
...
int Graph::mstPrim(const string &startLabel,
                   void visit(const string &from, const string &to,
                              int weight)) const {
  if(directionalEdges) { return -1;
}
  if(adjNodes.count(startLabel) == 0) { return -1;
}
  set<string> visited;
  visited.insert(startLabel);
  int sum = 0;
